#!/usr/bin/env python3
"""
Test Preprocessing Pipeline to TrOCR
Purpose: Load image, apply preprocessing (quality analysis + enhancement), run TrOCR, store results
...
def save_raw_results(results, output_dir, filename="trocr_raw_text.txt"):
...
def test_preprocessing_to_trocr(image_path=None):
    """Test complete pipeline: preprocessing + TrOCR"""
    print("=" * 80)
    print("PREPROCESSING TO TROCR TEST")
    print("=" * 80)
    print("Purpose: Quality analysis → Enhancement → TrOCR → Store results")
    print("-" * 80)
    ...
    output_dir = project_root / "preprocessed_and_trocr"
    ...
    try:
        ...
        if not trocr_engine.initialize():
            print("❌ TrOCR initialization failed")
            return False
        print("✅ TrOCREngine initialized")
        ...
        # STAGE 3: TrOCR Processing
        print("\n" + "=" * 60)
        print("STAGE 3: TROCR PROCESSING")
        print("=" * 60)

        ocr_start = time.time()
        ocr_results = trocr_engine.process_image(enhanced_image)
        ...
        print(f"❌ Error in preprocessing to TrOCR test: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test preprocessing pipeline to TrOCR')
    ...
    print("🧪 Testing preprocessing pipeline to TrOCR...")

    try:
        success = test_preprocessing_to_trocr(image_path)

        if success:
            print("\n✅ Test completed successfully!")
            print("📊 Preprocessing → Enhancement → TrOCR pipeline executed")
            print("💾 Results saved to preprocessed_and_trocr/ folder")
        else:
